...
An important counterintuitive fact about operator approval is the limited scope of the `isApproved` flag. Setting `isApproved` to `false` only prevents the operator from calling `createRail`. The other rail operations don't check `isApproved`: `modifyRailPayment` and `modifyRailLockup` use the `onlyRailOperator` modifier (which only checks that `msg.sender` matches the rail's stored `operator` address), while `terminateRail` runs its own check that authorizes either the operator or the payer directly. A revoked operator retains the ability to decrease rates, reduce fixed lockup, terminate rails, and make one-time payments from existing fixed lockup on any rails they previously created. They can also increase rates if remaining allowance headroom permits. Zeroing `rateAllowance` and `lockupAllowance` removes that increase headroom, but does not affect the decrease, termination, or one-time-payment paths, none of which consult the allowance. To fully neutralize a revoked operator on an existing rail, the payer must terminate the rail.
...
If the rail has a validator, termination also calls `IValidator.railTerminated()` as a notification callback. This call is **not** wrapped in try-catch, so if the validator reverts, the entire `terminateRail` transaction fails. This means a fully adversarial validator can block termination, which in turn makes `settleTerminatedRailWithoutValidation` unreachable (since it requires the rail to be terminated first). The combination of a validator that reverts on both `validatePayment()` and `railTerminated()` can permanently trap payer funds in the rail. Service contracts implementing `IValidator` should ensure `railTerminated()` does not revert.
...
**Important caveat on lockupUsage calculation during finalization**: Finalization assumes that `lockupUsage` for streaming lockup equals `rail.paymentRate × rail.lockupPeriod`. This assumption holds for rails whose rate was never modified after termination. However, if the operator reduces the rate during the grace period (permitted on terminated rails), a mismatch can occur because `modifyRailPayment` adjusts `lockupUsage` using the remaining epochs (`effectiveLockupPeriod`) while finalization uses the full `lockupPeriod` with the final rate. The phantom `lockupUsage` per rate reduction is exactly `(oldRate - newRate) × (lockupPeriod - remainingEpochs)`. Multiple rate reductions on the same rail compound the phantom, and reducing the rate to zero produces the maximum phantom for the elapsed epochs. In the worst case, accumulated phantom usage across multiple rails can prevent the operator from creating new rails even though all old rails are fully finalized. See [issue #274](https://github.com/FilOzone/filecoin-pay/issues/274) for details. Setting allowances with generous headroom mitigates this.
