...
An important counterintuitive fact about operator approval is the limited scope of the `isApproved` flag. Setting `isApproved` to `false` only prevents the operator from calling `createRail`. All other rail operations (`modifyRailPayment`, `modifyRailLockup`, `terminateRail`) use the `onlyRailOperator` modifier, which checks that `msg.sender` matches the rail's stored `operator` address, it does not check `isApproved`. A revoked operator retains the ability to decrease rates, reduce fixed lockup, terminate rails, and make one-time payments from existing fixed lockup on any rails they previously created. They can also increase rates if remaining allowance headroom permits. To fully restrict a revoked operator, the payer must also zero out `rateAllowance` and `lockupAllowance`.
